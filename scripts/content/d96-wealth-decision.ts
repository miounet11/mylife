// v5-D96 (2026-05-24): 财富 × 命理决策 SEO 长文批量生产
//
// 复用 D94 模板（引擎事实包 + LLM 修饰）。
// 8 个财富场景 × 6 个核心决策 = 48 篇中文 published；后续 D97 跑多语种翻译。
//
// 用法：
//   npx tsx scripts/content/d96-wealth-decision.ts
//   DRY=1 ONLY="property-buying-buy-or-wait" npx tsx scripts/content/d96-wealth-decision.ts
...
// 8 个财富场景（覆盖买房/创业资金/股市/理财/借贷/家族继承/合伙/破产应急）
const SCENES = [
  { key: 'property-buying', label: '买房决策', shortLabel: '买房', focus: '首付来源、城市选择、贷款年限、楼盘信号、夫妻共同' },
  { key: 'startup-funding', label: '创业资金筹措', shortLabel: '创业资金', focus: '自有资金/借款/天使融资、回报期、合伙股权、家庭资金' },
  { key: 'stock-investing', label: '股市与基金投资', shortLabel: '股市投资', focus: '风险偏好、择时进场、止损、配置比例、长线 vs 波段' },
  { key: 'long-term-saving', label: '长期理财与配置', shortLabel: '长期理财', focus: '保险/年金/REITs/海外资产、抗通胀、家庭目标储备' },
  { key: 'borrowing-and-debt', label: '借贷与债务', shortLabel: '借贷负债', focus: '亲友借款、信用卡、消费贷、车贷房贷、债务重组' },
  { key: 'inheritance', label: '家族继承与赠与', shortLabel: '家族继承', focus: '父母资产分配、兄弟姐妹博弈、企业接班、税务安排' },
  { key: 'partnership-money', label: '合伙与股权', shortLabel: '合伙股权', focus: '股权比例、入伙时机、退伙清算、家人入伙、信任崩盘' },
  { key: 'financial-crisis', label: '破产与财务危机应急', shortLabel: '财务危机', focus: '现金流断裂、负债爆雷、生活降级、心理重建、东山再起' },
];

// 6 个核心决策（每场景共用）
const DECISIONS = [
  { key: 'go-or-wait', label: '行动还是等待', topic: '财富', q: '在这个场景下，命盘里的哪些组合是"该出手"信号？哪些是"该缓一缓"信号？' },
  { key: 'best-timing', label: '最佳时间窗口', topic: '财富', q: '什么大运/流年/月支组合下，这个财务决策成功率显著高？' },
  { key: 'risk-tolerance', label: '风险承受度', topic: '财富', q: '命盘里的财星/比劫/印星结构怎么提示当事人真实的风险胃口（不是嘴上说的）？' },
  { key: 'partner-and-money', label: '合作对象判断', topic: '财富', q: '在这个场景里要找合伙人/借款人/出资人时，命盘怎么辅助判断对方可信度？' },
  { key: 'red-flag-signal', label: '危险信号识别', topic: '财富', q: '出现什么命盘组合或现实信号时，这个财务决策应该立刻停手？' },
  { key: 'recovery-strategy', label: '损失后的复盘与重启', topic: '财富', q: '已经亏了之后，命盘里什么样的组合提示该认赔出场？什么样的组合还可以补救？' },
];
...
const SYSTEM_PROMPT = `你是世界易学（World Yi）资深主编，特别擅长把命理判断接到具体财务场景里。给你一个【财富场景 × 决策类型】组合 + 3 个引擎跑出的真实虚拟命盘事实包，请产出一篇高质量长篇 SEO 知识词条。

【铁律】
1. 关于命理判断（日主/格局/用神/大运/流年/十神 — 尤其财星/比劫/印星组合），只能引用我提供的"命盘事实清单"，不允许编造清单外的命理结论。
2. 提到具体四柱/格局/用神时必须与清单对得上，不能换字。
3. 这是"财富 × 决策"主题，行文要兼顾 (a) 中国当代财务现实（房价/利率/通胀/家庭储蓄/亲友借贷/股市波动）和 (b) 命理结构如何放大或抑制每个决策点的风险。
4. 不能用算命腔说"必发横财""注定破产"，要用"在这种结构下，X 类信号出现时该怎么操作"的判断顺序语言。

【输出 JSON 结构】
{
  "title": "20-32 字，命中关键词，体现财富场景 + 决策类型，专业不浮夸",
  "excerpt": "80-140 字摘要，第一句直接说这个场景做这种决策的核心难点",
  "seoTitle": "≤56 字 SEO 标题，命中场景 + 决策关键词",
  "seoDescription": "120-160 字 SEO 描述",
  "tags": ["财富场景","决策主题","命理工具","世界易"],
  "sections": [
    { "title": "为什么这个决策在这个财富场景尤其难", "paragraphs": ["...3-4 段：现实约束 + 心理偏差 + 命理结构同时叠加..."] },
    { "title": "命理判断的三个核心维度", "paragraphs": ["...3-4 段：日主与财星 / 比劫与合作 / 流年大运冲合..."] },
...
2. 把这个命盘放进"${spec.scene.label}"的真实财务现实里
...
async function main() {
  ...
  if (!apiKey) { console.error('[d96] API_KEY missing'); process.exit(1); }
  const model = getModelFallbackChain(undefined, 'content')[0];
  if (!model) { console.error('[d96] no model'); process.exit(1); }
  ...
  console.log(`[d96] specs total=${allSpecs.length} skip_existing=${allSpecs.length - specs.length} todo=${specs.length} model=${model} concurrency=${CONCURRENCY} dry=${DRY}`);
  if (!specs.length) { console.log('[d96] nothing to do'); return; }
  ...
  for (const r of results) {
    ...
    try {
      saveManagedContentEntry({
        ...
        tags: r.payload.tags.length ? r.payload.tags : [r.spec.scene.shortLabel, r.spec.decision.topic, '财富', '世界易'],
        featured: false,
        ...
        status: 'published',
        source: 'engine-llm:wealth-decision',
        meta: {
  ...
  console.log(`[d96] ok=${ok} fail=${fail} saved=${saved} / ${specs.length} in ${(elapsed / 1000).toFixed(1)}s`);
}

main().catch((err) => {
  console.error('[d96] fatal:', err);
  process.exit(1);
});
