...
function main() {
  ...
  const drafts = listManagedContentEntries()
    ...
    .filter((entry) => entry.meta?.sourceType === 'public-growth-global')
    .filter((entry) => !key || entry.meta?.growthPlanKey === key)
    .map((entry) => ({
      entry,
      assessment: assessGrowthPublication(entry, 'public-growth-global'),
    }))
  ...
  const publishedEntries = ready.map(({ entry, assessment }) => (
    ...
    }, 'system_public_growth_global_promote')
  )).filter(Boolean);
