...
const targets = require('../data/public-growth-targets-global.json');
...
const coverage = targets.map((target) => {
  const matched = rows
    ...
    .filter((row) => row.meta.sourceType === 'public-growth-global');
...
const draftEntries = rows
  ...
  .map((row) => ({
    ...
    source: row.source,
    assessment: assessDraftPublication(row, 'public-growth-global'),
  }))
