        ...
        <div className="mt-5 grid gap-3">
          <label className="text-sm font-semibold text-[color:var(--ink)]">
            ...
            <select
              ...
              className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2"
            >
          ...
          <label className="text-sm font-semibold text-[color:var(--ink)]">
            名称
            <input value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2" />
          </label>
          <label className="text-sm font-semibold text-[color:var(--ink)]">
            Base URL
            <input value={form.baseUrl} onChange={(event) => setForm((current) => ({ ...current, baseUrl: event.target.value }))} className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2" />
          </label>
          <label className="text-sm font-semibold text-[color:var(--ink)]">
            模型
            <input value={form.model} onChange={(event) => setForm((current) => ({ ...current, model: event.target.value }))} className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2" />
          </label>
          <label className="text-sm font-semibold text-[color:var(--ink)]">
            API Key
            <input type="password" value={form.apiKey} onChange={(event) => setForm((current) => ({ ...current, apiKey: event.target.value }))} placeholder="留空则保留原 key" className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2" />
          </label>
          <div className="grid gap-3 sm:grid-cols-3">
            <label className="text-sm font-semibold text-[color:var(--ink)]">
              优先级
              <input value={form.priority} onChange={(event) => setForm((current) => ({ ...current, priority: event.target.value }))} className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2" />
            </label>
            <label className="text-sm font-semibold text-[color:var(--ink)]">
              超时 ms
              <input value={form.timeoutMs} onChange={(event) => setForm((current) => ({ ...current, timeoutMs: event.target.value }))} className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2" />
            </label>
            <label className="text-sm font-semibold text-[color:var(--ink)]">
              重试
              <input value={form.maxRetries} onChange={(event) => setForm((current) => ({ ...current, maxRetries: event.target.value }))} className="mt-2 w-full rounded-xl border border-[color:var(--line)] bg-white px-3 py-2" />
            </label>
          </div>
          ...
          {message ? <div className="rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">{message}</div> : null}
