...
export function ReportWorldYiDual({
  ...
  const layers = compact
    ? reading.layers.filter((layer) => layer.id === 'structure' || layer.id === 'timing' || layer.id === 'action')
    : reading.layers;
  ...
  return (
    <section
      ...
      className={
        compact
          ? 'scroll-mt-header rounded-[var(--radius)] border border-[color:var(--hairline)] bg-white p-3'
          : 'scroll-mt-header rounded-[var(--radius-md)] border border-[color:var(--hairline)] bg-white p-4 md:p-5'
      }
    >
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <h2
            className={
              compact
                ? 'mt-0.5 text-[14px] font-semibold text-[color:var(--ink-1)]'
                : 'mt-1 text-[16px] font-semibold text-[color:var(--ink-1)]'
            }
          >
            {title || '易学事实 · 世界易判断'}
          </h2>
      ...
      <div className={compact ? 'mt-3 space-y-3' : 'mt-4 grid gap-4 lg:grid-cols-2'}>
        <article className={compact ? 'rounded-md border border-[color:var(--hairline)] p-2.5' : 'rounded-lg border border-[color:var(--hairline)] p-3.5'}>
        ...
        <article className={compact ? 'rounded-md border border-[color:var(--brand-soft-2)] bg-[color:var(--brand-soft)]/40 p-2.5' : 'rounded-lg border border-[color:var(--brand-soft-2)] bg-[color:var(--brand-soft)]/40 p-3.5'}>
      ...
      {compact ? null : <p className="mt-3 text-[12px] leading-relaxed text-[color:var(--ink-4)]">{reading.motherTongue}</p>}
      <p className={compact ? 'mt-2 text-[11px] text-[color:var(--ink-5)]' : 'mt-1 text-[11px] text-[color:var(--ink-5)]'}>
        {reading.refuse}
      </p>
    </section>
  );
