  ...
  create(input: CreateFortuneInput): FortuneRecord {
    ...
    this.execute(
      `INSERT INTO fortunes (
        ...
        analysis, kline_data, dayun, shen_sha, report_version, is_public, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        input.id,
        input.userId,
        ...
        input.dayun ? JSON.stringify(input.dayun) : null,
        input.shenSha ? JSON.stringify(input.shenSha) : null,
        input.reportVersion || 'v1',
