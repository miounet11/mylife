...
export function buildPersonalGrowthHub(params: {
  ...
  const primaryToolContinuationLabel = primaryTool?.premiumServiceKey
    ...
    : 'AI 深问';
  // v5-D58 (2026-05-21): 配合 D57 C 端去付费方向，urgencyLine 不再说「付费承接」
  const urgencyLine = premiumOffer
    ? `当前最值得继续推进的方向是 ${premiumOffer.title}，因为它能把"知道一点"升级成"真正能做决定"。`
    : primaryTool
      ? `当前最自然的下一步是 ${primaryToolContinuationLabel}，把这次的判断继续往里推一层。`
      : '先建立一次可复用的个人判断记录，后面所有工具和内容才会越用越准。';

  return {
    heading: '下一步，不该只停在浏览',
    description: hasPersonalSignal
      ? '系统已经有你最近的报告和工具历史。这里把你带回最该继续的报告、工具、文章、案例和下一步动作。'
