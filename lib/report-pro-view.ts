...
function buildOverviewSections(
  ...
  sections.push({
    ...
    body:
      [
        ...
        ...topicParas,
        ...domainExtra,
        '下方议题卡片默认展开「结论 / 为什么 / 怎么做 / 你可能想问」，可逐条深读或点顾问开场。',
      ]
        .filter(Boolean)
        .join('\n\n') || '四维细节见下方议题卡片，可逐条展开。',
  ...
  sections.push({
    ...
    body: `有疑问很正常。下面先扫一遍常见卡点；若仍不解，点任意议题的「顾问开场」，用你的生活问题来问即可。\n\n${faqBody}`,
...
export const PRO_TERM_GLOSSARY: Record<string, string> = {
  用神: '扶抑主线：身弱喜印比生扶，身旺喜克泄。对你最该优先靠的方向，不是求签标签。',
  喜神: '辅助主用神的有利因素；调候五行也常放在喜神侧说明。',
  忌神: '容易消耗你的方向，大事上尽量少硬碰——不是诅咒。',
  调候: '季节温度调节（如冬火夏水），单独看，不替代扶抑主用神。',
