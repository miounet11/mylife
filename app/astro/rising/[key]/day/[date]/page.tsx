...
export function generateStaticParams() {
  ...
  for (const r of RISING_PROFILES) {
    for (const date of dates) out.push({ key: r.key, date });
  }
  return out;
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const pack = buildAstroDailyMatchPack(date, { kind: 'rising', key: key as SignKey });
  if (!pack) return { title: '上升日运' };
  return buildPageMetadata({
    title: pack.seo.title,
    description: pack.seo.description,
    path: `/astro/rising/${key}/day/${date}`,
    type: 'article',
    keywords: pack.seo.keywords,
  });
}

export default async function RisingDayPage({ params }: Props) {
  const { key, date } = await params;
  if (!getRisingByKey(key)) notFound();
  const pack = buildAstroDailyMatchPack(date, { kind: 'rising', key: key as SignKey });
  if (!pack) notFound();
  const path = `/astro/rising/${key}/day/${date}`;

  return (
    <AppPage header={{ ctaHref: pack.bridges.analyze, ctaLabel: '结构报告', compact: true }}>
      <AnalyticsPageView
        eventName="astro_rising_day"
        page={path}
        meta={{ rising: key, date, score: pack.scores.composite }}
      />
      <JsonLd
        data={buildBreadcrumbJsonLd([
          ...
          { name: pack.identity.title, path: `/astro/rising/${key}` },
          { name: date, path },
        ])}
      />
      <JsonLd
      ...
      <div className="page-content py-6 pb-16 md:py-8">
        <AstroDailyMatchView
          ...
          breadcrumbs={[
            { href: '/astro', label: '星座' },
            { href: '/astro/rising', label: '上升' },
            { href: `/astro/rising/${key}`, label: pack.identity.title },
