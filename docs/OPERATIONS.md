...
- `MODEL_FALLBACK_CHAIN`：默认 `auto,gpt-5.2`
- `REPORT_MODEL_FALLBACK_CHAIN`：默认 `auto,gpt-5.2`
- `REPORT_NARRATIVE_MODEL_FALLBACK_CHAIN`：默认 `auto,gpt-5.2`
- `CONTENT_GENERATION_MODEL_FALLBACK_CHAIN`：默认 `auto,gpt-5.2`

### 图片生成

- `VISUAL_ASSET_API_BASE_URL`：默认 `https://www.gemiai.top/v1`
- `VISUAL_ASSET_API_KEY`：图片供应商密钥，只放运行环境
- `VISUAL_ASSET_DEFAULT_MODEL`：默认 `gpt-image-2`
- `VISUAL_ASSET_CORE_MODEL`：默认 `gpt-image-2-pro`
- `VISUAL_ASSET_NARRATIVE_MODEL`：默认 `grok-420-fast`
- `VISUAL_ASSET_CONCURRENCY_LIMIT`：默认 `6`
- `VISUAL_ASSET_GENERATION_TIMEOUT_MS`：默认 `180000`
